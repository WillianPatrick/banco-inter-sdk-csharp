...
public class BancoInter
{
    ...
    /// </summary>
    public async Task<Jwt> ObterTokenAsync(string[] scopes = null)
    {
        ...
        if (!response.IsSuccessStatusCode)
        {
            ...
            throw new Exception($"Erro ao obter token de autenticação. Código: {response.StatusCode}, Detalhe: {erro}");
        }
    ...
    public async Task<string> EmitirBoletoAsync(Payments.DTOs.RequisicaoEmitirCobranca cobranca)
    {
        ...
        requestMessage.Headers.Authorization = new AuthenticationHeaderValue("Bearer",_authToken.Token);
    ...
    public async Task<Payments.DTOs.ConsultaCobrancaResponse> ConsultarCobrancaAsync(string codigoSolicitacao)
    {
        ...
        request.Headers.Authorization = new AuthenticationHeaderValue("Bearer", _authToken.Token);
